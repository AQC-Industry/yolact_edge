...
def preprocess_crop(paths, preprocess):
    save_path = ".tmp/"
    ...
    for path in paths:
        ...
        if preprocess:
            im = clahe(im)
        im_name = os.path.basename(path).split(".")[0]
...
def extract_contour_and_bbox(mask, apply=False, width=4096, height=1024): #TODO What to do with weird masks??
    ...
    if apply:
        ...
        epsilon = 0.01*cv.arcLength(best_contour,True)
        approx = cv.approxPolyDP(best_contour,epsilon,True)
        approx = [tuple([x[0][0]/width, x[0][1]/height]) for x in approx]
        # Bonding box
        x,y,w,h = cv.boundingRect(best_contour)
        ...
        for cat in categories:
            ...
            if mask.max() > 0:
                ...
                if cat == 9: #only borders
                    ...
                    kernel = np.ones((5,5),np.uint8) #20
...
def main(multithread: bool = False, preprocess: bool = False) -> None:
    ...
    preprocess_crop(eval_paths, preprocess)
...
if __name__ == '__main__':
    main(multithread=True, preprocess=True)